...
export function balanceEquation(equation: string): {
  ...
  const formatCompound = (compound: string, coeff: number): string => {
    if (coeff === 1) return compound;
  ...
  const balancedProducts = products.map((p, i) =>
    formatCompound(p, coefficients[reactants.length + i])
  );
 ...
 */
function solveNullSpace(matrix: number[][], numVars: number): number[] | null {
